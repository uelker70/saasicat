// useSubscriptionDraft — reaktiver Konfigurations-State + abgeleitete Preise.
// Tests gegen das gebaute Bundle (`dist/index.js`) ohne DOM/Vue-Test-Utils —
// das Composable ist HTTP-frei und reactivity-only, also direkt aufrufbar.
...
function buildDraft(overrides = {}) {
    return useSubscriptionDraft({
        ...
        // explizit-null muss erlaubt sein → nicht via `??` defaulten
...
describe('useSubscriptionDraft — Plan-Auswahl', () => {
    test('selectedPlan ist null vor Auswahl', () => {
        const d = buildDraft({ initialPlan: null });
        assert.equal(d.selectedPlan.value, null);
        assert.equal(d.pricing.value.planNet, 0);
    });

    test('setPlan entfernt Bundles, die mit dem neuen Plan inkompatibel sind', () => {
        ...
        assert.equal(
            ...
            false,
            'NUR_SPORT ist nur mit SPORT kompatibel → muss entfernt werden',
        );
        assert.equal(d.selectedPlan.value.id, 'STARTER');
    });

    test('setPlan behält universell kompatible Bundles', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
...
describe('useSubscriptionDraft — Cycle-Toggle', () => {
    test('Monthly nutzt monthlyNet, Yearly nutzt yearlyNet', () => {
    ...
    test('yearlyNet=null fällt auf monthly × DEFAULT_YEARLY_FACTOR zurück', () => {
        const plansNoYearly = [
            {
                id: 'X',
                name: 'X',
...
describe('useSubscriptionDraft — Bundles', () => {
    test('Bundle-Toggle markiert Bundle + aktiviert dessen Features', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
        d.toggleSubscriptionBundle('bv-sport');
        assert.equal(d.selectedBundleVersionIds.value.has('bv-sport'), true);
        assert.equal(d.activeFeatures.value.has('TRAINING_PLANNER'), true);
        assert.equal(d.activeFeatures.value.has('TOURNAMENT'), true);
        // TEAMS ist auch in SPORT included — egal, aktive Set enthält's nur einmal
        assert.equal(d.activeFeatures.value.has('TEAMS'), true);
    });

    test('Bundle deselect entfernt aktivierte Features wieder', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
        d.toggleSubscriptionBundle('bv-sport');
        d.toggleSubscriptionBundle('bv-sport');
    ...
    test('Bundle-Preis fließt in subtotalNet ein', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
        const beforeNet = d.pricing.value.subtotalNet;
        d.toggleSubscriptionBundle('bv-sport');
        // SPORT 19.9 + Bundle 16.9 = 36.80 (×100 wegen FP-Drift)
...
describe('useSubscriptionDraft — Promo-Discount', () => {
    test('PERCENT-Promo wird auf subtotalNet angewendet', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
    ...
    test('ABSOLUTE-Promo wird gekappt bei subtotal', () => {
        ...
        // STARTER 9.90, ABSOLUTE 100 → gekappt auf 9.90, total = 0
        assert.equal(d.pricing.value.discountNet, 9.9);
        assert.equal(d.pricing.value.totalNet, 0);
    });

    test('clearPromo entfernt Discount + setzt status idle', () => {
        const d = buildDraft();
        d.setPromoCode('X');
    ...
    test('setPromoCode räumt einen früheren valid-Status auf', () => {
        const d = buildDraft();
        d.setPromoCode('OLDCODE');
        d.setPromoState({ status: 'valid', preview: null, message: 'OK' });
        d.setPromoCode('NEWCODE');
        // Status muss zurück auf idle, weil der Tenant neuen Code tippt
        assert.equal(d.promoState.value.status, 'idle');
    });
});

describe('useSubscriptionDraft — toApiPayload', () => {
    test('serialisiert Plan + Cycle + Bundle-Version-IDs', () => {
    ...
    test('ohne Bundle-Auswahl fehlt bundleVersionIds im Payload', () => {
        const d = buildDraft({ initialPlan: 'SPORT' });
        assert.equal(d.toApiPayload().bundleVersionIds, undefined);
    });

    test('serialisiert promoCode nur bei status valid', () => {
        const d = buildDraft();
        d.setPromoCode('TEST20');
        // status idle → kein promoCode
    ...
    test('wirft, wenn Plan nicht gesetzt ist', () => {
        const d = buildDraft({ initialPlan: null });
        assert.throws(() => d.toApiPayload(), /plan ist nicht gesetzt/);
    });
});

describe('useSubscriptionDraft — redundante (gedeckte) Bundles', () => {
    // Repro: Plan STANDARD={features:[C]}-frei; Y={C}, Z={C,D}. Y wird durch
    // Plan ∪ Z vollständig gedeckt → darf weder berechnet noch gesendet werden.
    const STANDARD = {
        id: 'STANDARD',
        name: 'Standard',
        tagline: '',
    ...
    test('gedecktes Bundle fließt nicht in bundlesNet und nicht in den Breakdown', () => {
        const d = buildOverlapDraft();
        d.toggleSubscriptionBundle('bv-y');
        d.toggleSubscriptionBundle('bv-z');
        // Nur Z wird berechnet (7), Y ist durch Z gedeckt.
    ...
    test('gedecktes Bundle fehlt in toApiPayload().bundleVersionIds', () => {
        const d = buildOverlapDraft();
        d.toggleSubscriptionBundle('bv-y');
        d.toggleSubscriptionBundle('bv-z');
    ...
    test('Abwählen des deckenden Bundles berechnet das andere wieder', () => {
        const d = buildOverlapDraft();
        d.toggleSubscriptionBundle('bv-y');
        d.toggleSubscriptionBundle('bv-z');
        assert.equal(d.pricing.value.bundlesNet, 7);
        ...
        d.toggleSubscriptionBundle('bv-z');
        // Y steht allein → wieder berechnet + gesendet.
        assert.equal(d.pricing.value.bundlesNet, 4);
        assert.deepEqual(d.toApiPayload().bundleVersionIds, ['bv-y']);
    });

    // Gegenseitige Deckung: Y={C} und Z={C} decken sich gegenseitig. Ein
    // einmaliger Filter würde BEIDE verwerfen (Unter-Berechnung + verlorenes
    // Entitlement für C). Die iterative Auswahl behält genau EINS.
    test('gegenseitige Deckung Y={C},Z={C} → genau EIN Bundle berechnet + gesendet', () => {
        const C_ONLY = {
            bundleKey: 'C2',
            label: 'Bundle C2',
            bundleVersionId: 'bv-z',
        ...
        // Genau EIN Bundle bleibt (nicht null) — deterministisch bv-z
        // (bundleVersionId asc → bv-y zuerst geprüft + verworfen).
        const ids = d.toApiPayload().bundleVersionIds;
        assert.equal(ids.length, 1);
        assert.deepEqual(ids, ['bv-z']);
        assert.equal(d.pricing.value.bundlesNet, 7);
        assert.equal(d.pricing.value.breakdown.bundles.length, 1);
        // Feature C bleibt aktiv (Entitlement nicht verloren).
        assert.equal(d.activeFeatures.value.has('C'), true);
    });
});

describe('useSubscriptionDraft — isDirty', () => {
    test('false bei frischem State', () => {
        const d = buildDraft();
        assert.equal(d.isDirty.value, false);
    });

    test('true sobald ein Bundle zugebucht wird', () => {
        const d = buildDraft();
        d.toggleSubscriptionBundle('bv-sport');
