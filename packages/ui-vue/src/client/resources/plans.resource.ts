...
/** The plan catalogue: the stem list, its tenant counts, and the stem CRUD. */
export const plansResource = defineResource('plans', {
    list: async (http, ctx): Promise<PlanRow[]> =>
        (await requestJson<PlanRow[]>(http, plansUrl(ctx))) ?? [],

    tenantCounts: async (http, ctx): Promise<Record<string, number>> =>
        (await requestJson<Record<string, number>>(http, `${plansUrl(ctx)}/tenant-counts`)) ?? {},
