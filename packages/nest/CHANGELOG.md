...
- 07c30c6: A contract's lines add up to its totals, and a promotion lowers a price and
  nothing else
...
- 877faa4: The public catalogue says what a new payment method is taken with
...
- f582840: A checkout offer is priced from the catalogue; a caller only chooses
...
- d8721ab: Every operator route `SaaSiCatModule.forRoot` mounts requires the platform
  administrator
...
- d8721ab: Lasting operator actions on the platform's own routes require the second factor
...
- a512a48: `dbCatalog` names the file; it no longer takes the settings as values
...
- 7c7b06c: A configuration change is reported to the people the file names
...
- 6d9a238: A value in `config/saas.yaml` may name an environment variable
...
- 16a8542: The notice period and the self-service plan blocks move into `config/saas.yaml`
...
- a3a298e: Booking an add-on now asks for the tenant administrator role
