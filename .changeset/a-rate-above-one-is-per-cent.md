...
- Where the totals explain both readings or neither, a rate of 0 is recorded
  as 0, which it is in either unit, and a rate above 1 is per cent: as a
  fraction it would be a tax above 100 per cent. A rate of exactly 1 is still
  100 per cent as a fraction, so the size does not settle it.
- Above 0 and up to 1, provenance decides only where both readings explain the
