...
| `cancelSubscription( tenantId: string, input: { canceledAt: Date; effectiveAt: Date; terminateNow: boolean }, ): Promise<{ canceledAt: Date \| null; canceledEffectiveAt: Date \| null; status: string }>` | Record a cancellation.                                                                                                                             |
